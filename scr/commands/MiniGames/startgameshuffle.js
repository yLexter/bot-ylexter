...
const execute = async (client, msg, args, cor) => {
  ...
  try {
    ...
    if (shuffle) {
      ...
      const errorMsg = () => {
        return msg.reply({content: '`Utilize comando + configs para ver os parametros válidos | *Comando = gamesh`'})
      }
      ...
      if (!verify) {
        let string = ''
        configsGame.forEach(element => {
          string += `**Config**: ${element.name}\n**Help:** ${element.help}\n\n`
        })
    ...
    async function game() {
      ...
      collector.on('collect', m => {
        ...
        shuffle.rodada++
        client.shuffles.set(msg.guild.id, shuffle)
        const verify = players.find(element => element.pontos == shuffle.questionsWin)
        const string_1 = players_ordenados()

        const helpMsg = new MessageEmbed()
          .setColor(cor)
          .setDescription(`${string_1}`)
      ...
      collector.on('end', async collected => {
        ...
        const shuffle = client.shuffles.get(msg.guild.id)
        const champion = shuffle.players.find(element => element.pontos == shuffle.questionsWin)
        const afk = 10
